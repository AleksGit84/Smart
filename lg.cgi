...
sub read_config {
	...
	$xp->parsefile("/media/data/www/stats.in.soho.net.ua/smart/lg.conf");
	undef($xp);
}
...
print "</style>\n";
	...
	print "</HEAD>\n";
	print "<BODY BGCOLOR=\"#FFFFFF\" TEXT=\"#000000\" font-family=\"Tahoma\">\n";
#	if ($logoimage ne "") {
...
#		print "</TD></TR></TABLE>\n";
#	}
#$headr = `/usr/bin/php -f /media/data/www/stats.in.soho.net.ua/smart/header.php`;
#print $header;
#print "test" ;
#print "test";
	print "<CENTER>\n";
#	print "<H2>$titlestr</H2>\n";
	print "<H2>SoHo.NET Looking Glass</H2>\n";
	print "</CENTER>\n";
	print "<P>\n";
	print "<HR SIZE=2 WIDTH=\"100%\" NOSHADE>\n";
	print "<P>\n";
}
...
<TR>
<TH BGCOLOR="#b6dcee" NOWRAP><FONT COLOR="#000000">Type of Query</FONT></TH>
<TH BGCOLOR="#b6dcee" NOWRAP><FONT COLOR="#000000">Additional parameters</FONT></th>
<TH BGCOLOR="#b6dcee" NOWRAP><FONT COLOR="#000000">Node</FONT></TH></TR>
<TR><TD>
<TABLE BORDER=0 CELLPADDING=2 CELLSPACING=2>
...
<TR><TD><INPUT TYPE="radio" NAME="query" VALUE="summary" CHECKED></TD><TD>&nbsp;bgp&nbsp;summary</TD></TR>
...
sub print_results
{
	...
	while (1) {
		...
		if ($command eq "show ip bgp summary") {
			...
			my $ip = $1;
			if ($securemode) {
				s/^(Peer:\s+[\d\.:]+\+)\d+(\s+AS\s+\d+\s+Local:\s+[\d\.:]+\+)\d+(\s+AS\s+\d+)/${1}???${2}???${3}/g;
			}
