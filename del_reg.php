...
function go_back()
{
    ...
    print ("<TBODY>\n");
    print ("<META http-equiv=\"refresh\" content=\"1;url=manager.php?switch_segment=" . $_GET['switch_segment'] . "&switch_ip_address=" . $_GET['switch_ip_address'] . "&switch_address_t2=" . urlencode($_GET['switch_address_t2']) . "\">\n");
    print ("<tr>\n");
...
//	  	mysql_query("DELETE FROM portstatus WHERE `switch_id`=".$switch_id." AND `port_id`=".$port_id." AND `mac_cur`='".$mac_cur."'");
        mysqli_query("DELETE FROM portstatus WHERE `switch_id`=" . $switch_id . " AND `port_id`=" . $port_id . " AND `mac_cur`='" . $mac_cur . "'");
...
if ($cmd == "delete") {
    ...
    if (isset($switch_id) and isset($port_id) and isset($mac)) {
        $query_delete_mac = "DELETE FROM portstatus WHERE `switch_id`=$switch_id AND `port_id`=$port_id AND `mac`=\"$mac\"";
//	$results_delete_mac = mysql_query($query_delete_mac) or die("Query query_delete_mac failed");
        $results_delete_mac = mysqli_query($query_delete_mac) or die("Query query_delete_mac failed");
...
//	header("Location: manager.php");
    ...
    if (isset($switch_id) and isset($port_id) and isset($mac_cur)) {
        ...
        $query_delete_mac_cur = "DELETE FROM portstatus WHERE `switch_id`=$switch_id AND `port_id`=$port_id AND `mac_cur`=\"$mac_cur\"";
//	$results_delete_mac_cur = mysql_query($query_delete_mac_cur) or die("Query query_delete_mac_cur failed");
        $results_delete_mac_cur = mysqli_query($query_delete_mac_cur) or die("Query query_delete_mac_cur failed");
...
//	header("Location: manager.php", false );
    ...
    if (isset($qty_cur)) {
        $query_delete_mac_cur = "DELETE FROM portstatus WHERE `mac_cur`<>\"\" AND `qty_cur`=$qty_cur";
//        $results_delete_mac_cur = mysql_query($query_delete_mac_cur) or die("Query query_delete_mac_cur failed");
        $results_delete_mac_cur = mysqli_query($query_delete_mac_cur) or die("Query query_delete_mac_cur failed");
        echo "DELETE OK!";
//	header("Location: manager.php");
    ...
    if (isset($switch_id) and !isset($port_id) and !isset($mac) and !isset($mac_cur)) {
        $query_delete_all_mac = "DELETE FROM portstatus WHERE `switch_id`=$switch_id";
//        $results_delete_all_mac = mysql_query($query_delete_all_mac) or die("Query query_delete_all_mac failed");
        $results_delete_all_mac = mysqli_query($query_delete_all_mac) or die("Query query_delete_all_mac failed");
...
if ($cmd == "register") {
    ...
    # REGISTER MAC
    $query_register_mac_cur = "UPDATE portstatus SET `mac` = \"$mac_cur\", `ip` = \"$ip_cur\", `mac_date` = $mac_date_cur, `mac_cur` = \"\", `ip_cur` = \"\", `mac_cur_date` = 0, `qty_cur` = 0 WHERE `switch_id`=$switch_id AND `port_id`=$port_id AND `mac_cur`=\"$mac_cur\"";
//    $results_register_mac_cur = mysql_query($query_register_mac_cur) or die("Query query_register_mac_cur failed");
    $results_register_mac_cur = mysqli_query($query_register_mac_cur) or die("Query query_register_mac_cur failed");
...
if (isset($_GET['switch_view'])) {
    ...
    $query_switch_view_update = "UPDATE switches SET `switch_view` = $switch_view WHERE $where_switch_id";
//    $results_switch_view_update = mysql_query($query_switch_view_update) or die("Query query_switch_view_update failed");
    $results_switch_view_update = mysqli_query($query_switch_view_update) or die("Query query_switch_view_update failed");
//    header("Location: manager.php");
//	go_back();
}
?>