...
<?php
print ( "<a href=manager.php?switch_segment=".$_GET['switch_segment']."&switch_ip_address=".$_GET['switch_ip_address']."&switch_address_t2=".urlencode( $_GET['switch_address_t2'] )."&switch_view=1><b>Show</b></a>\n" );
print ( "<a href=manager.php?switch_segment=".$_GET['switch_segment']."&switch_ip_address=".$_GET['switch_ip_address']."&switch_address_t2=".urlencode( $_GET['switch_address_t2'] )."&switch_view=0><b>Hide</b></a>\n" );
print ( "<a href='manager.php?switch_segment=".$_GET['switch_segment']."&switch_ip_address=".$_GET['switch_ip_address']."&switch_address_t2=".urlencode( $_GET['switch_address_t2'] )."&cmd=delete&qty_cur=1#switch1' onClick=\"return confirm('Are you sure you want to DELETE all UNREGISTERED MACs discovered not more than one time?');\"><img src='images/trash.gif' border=0 alt='DELETE all UNREGISTERED MACs discovered not more than one time'></a>\n" );
...
print ( "<td width=15% bgColor=#ffffff><b>Commands<br><input type=checkbox name=total value=checkbox onClick=\"checkAll(this.form,'ports[]',this.checked)\">Отметить все</b></td>\n" );
print ( "</tr>\n" );
print ( "</tbody>\n" );

$switch_num=0;

#Запрос к базе данных порезанный на куски, которые потом складываются. При задании переменных в запрос могут
#добавляться дополнительные части.
...
$query_switches_where = "
					WHERE (portstatus.mac='".$_GET['mac_registered']."' OR portstatus.mac_cur='".$_GET['mac_registered']."') AND portstatus.switch_id=switches.switch_id ";
$query_switches_order_by = "
					ORDER BY switches.switch_id";
...
#Добавление списка условий в запрос при наличии значений в переменных
if ( $_GET['switch_segment'] >= 0 )
{
	if ( $_GET['mac_registered'] ) { $add = "AND "; } else { $add = " WHERE "; }

	$query_switches .= $add."((switch_ip&65280)/256)='".$_GET['switch_segment']."' ";
	$query_switches_order_by .= ",switch_ip ";
}

if ( $_GET['switch_address_t2'] )
{
	if ( $_GET['mac_registered'] or $_GET['switch_segment'] >= 0) { $add = "AND "; } else { $add = " WHERE "; }
	$query_switches .= $add."switch_address='".urldecode( $_GET['switch_address_t2'] )."' ";
}

if ( $_GET['switch_ip_address'] )
{
	if ( $_GET['mac_registered'] or $_GET['switch_segment'] >= 0 or $_GET['switch_address_t2']) { $add = "AND "; } else { $add = " WHERE "; }
	$query_switches .= $add."switch_ip='".$_GET['switch_ip_address']."'";
}

$query_switches .= $query_switches_order_by;



#Передаём результат запроса в переменную или выдаём сообщение о неудачном запросе
//$results_switches = mysql_query($query_switches) or die("Query swicthes failed1".$query_switches);
$results_switches = mysqli_query($query_switches) or die("Query swicthes failed1".$query_switches);
...
while ($row_switches = mysqli_fetch_array( $results_switches, MYSQL_NUM )) {
    ...
    $results_switches_type = mysql_query($query_switches_type) or die("Query swicthes failed2");
    ...
    $mac_reg1 = $_GET['mac_registered'];
    if ( $_GET['mac_registered'] )
    {
    	$query_ports = "SELECT port_id, mac, ip, mac_date, mac_cur, ip_cur, mac_cur_date, qty_cur, comments FROM portstatus WHERE mac_cur='$mac_reg1' OR mac='$mac_reg1' AND `switch_id`=$switch_id ORDER BY `port_id` ASC";
    }
    else
    {
...
//    $results_ports = mysql_query($query_ports) or die("Query ports failed3");
//    $results_ports2 = mysql_query($query_ports) or die("Query ports failed4");
//    $mac_count = mysql_num_rows($results_ports);

    $results_ports = mysqli_query($query_ports) or die("Query ports failed3");
    $results_ports2 = mysqli_query($query_ports) or die("Query ports failed4");
...
//    mysql_free_result($results_ports2);
    ...
    echo "
    	...
    	<tr>
    		<td>";
#Скобка в конце $switch_num) не ошибка
    echo "
    	<b>
    	...
    	&nbsp&nbsp&nbsp
    	<a href='manager.php?switch_segment=".$_GET['switch_segment']."&switch_ip_address=".$_GET['switch_ip_address']."&switch_address_t2=".urlencode( $_GET['switch_address_t2'] )."&cmd=delete&switch_id=$switch_id#switch$switch_num' onClick=\"return confirm('Are you sure you want to DELETE all MACs?');\">
    ...
    if ( $switch_view == 0 ) {
		echo "<a href=manager.php?mac_registered=".$_GET['mac_registered']."&switch_segment=".$_GET['switch_segment']."&switch_ip_address=".$_GET['switch_ip_address']."&switch_address_t2=".urlencode( $_GET['switch_address_t2'] )."&switch_view=1&switch_id=$switch_id#switch$switch_num>
        <img src='images/closed.gif' border=0 alt='show'></a>";
    } else {
        echo "<a href=manager.php?mac_registered=".$_GET['mac_registered']."&switch_segment=".$_GET['switch_segment']."&switch_ip_address=".$_GET['switch_ip_address']."&switch_address_t2=".urlencode( $_GET['switch_address_t2'] )."&switch_view=0&switch_id=$switch_id#switch$switch_num>
...
echo "</TBODY></TABLE>";
echo '<div align=right><input type=submit value=Удалить></div></form>';
